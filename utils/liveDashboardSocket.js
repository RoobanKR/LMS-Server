...
function registerLiveDashboardHandlers(io, socket) {
  ...
  // ── Disconnected → after 30s with no reconnect, mark not-in-progress ─────────
  socket.on("student:disconnected", async ({ assessmentId, studentId }) => {
    try {
      if (!assessmentId || !studentId) return;
      await ExamSession.updateOne({ assessmentId, studentId }, { $set: { isOnline: false, lastActivityAt: new Date() } });
